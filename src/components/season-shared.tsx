...
export function StatCard(props: { label: string; value: string; tone?: "default" | "accent" }) {
  return (
    <div
      className={`ow-panel-light flex h-full flex-col px-5 py-5 ${
        props.tone === "accent"
    ...
    >
      <p
        className={`text-xs uppercase tracking-[0.22em] ${
          props.tone === "accent" ? "text-[#ffd39e]" : "text-[var(--ow-muted)]"
        }`}
      >
        {props.label}
      </p>
  ...
  return (
    <section className="ow-cut-panel p-5 text-[var(--ow-text)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="ow-title text-sm font-semibold uppercase">{props.title}</h3>
          {props.description ? (
