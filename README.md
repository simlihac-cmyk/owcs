...
- 공개 화면은 먼저 관리자 원본 파일을 기준으로 로드하고, 브라우저에 저장된 로컬 작업본이 있으면 그 상태를 이어서 표시합니다.
- 경기 결과, 규칙, 시즌 참가팀, 수동 rating과 예측 시나리오는 localStorage에 저장되어 새로고침 후에도 유지됩니다.
- 좌측 아카이브에서 과거/현재 시즌을 자유롭게 오가며 상세 정보를 다시 조회할 수 있습니다.
- `현재 기준으로 초기화` 버튼을 누르면 브라우저 작업본을 지우고 현재 관리자 원본 기준 상태로 되돌릴 수 있습니다.
...
- 기본 정렬 우선순위는 `승수 > 세트득실`입니다.
- 정확히 두 팀이 `승수 + 세트 득실`까지 같으면 승자승을 먼저 적용합니다.
- 그 밖의 잔여 동률은 시즌 설정의 타이브레이커 순서에 따라 정리합니다.
...
- `2024 스테이지 1`: 정규시즌 순위표를 기준으로 재구성한 과거 시즌 샘플입니다.
- `2024 스테이지 2`: 정규시즌 순위표와 경기 일람을 기준으로 재구성한 과거 시즌 샘플입니다.
...
- `2024 스테이지 1`: 순위표 기반 과거 시즌 샘플
- `2024 스테이지 2`: 순위표와 경기 일람 기반 과거 시즌 샘플
